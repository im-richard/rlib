...
/*
*   check :: ulx override enabled
*/

if not ulx or not cfg or not cfg.ulx_override then return end
...
function cmds.buildArgsList( cmd )
	...
	if LocalPlayer():query( cmd.cmd ) then
		local str_button = cmd.cmd
		if cmd.setButton and isstring( cmd.setButton ) then
			str_button = cmd.setButton
		end
		local panel = xlib.makebutton{ label=str_button, parent=cmds.argslist }
		panel.xguiIgnore = true
